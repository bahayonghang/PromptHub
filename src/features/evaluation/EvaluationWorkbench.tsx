...
const inputClass =
  "w-full rounded-sm border border-input bg-background px-2 py-1.5 text-label text-foreground outline-none";
...
export function EvaluationWorkbench({ prompt, versions }: EvaluationWorkbenchProps) {
  ...
  return (
    <div className="flex h-full min-w-0 flex-col bg-background">
        ...
        <div className="flex items-center gap-1" role="tablist" aria-label={t("evaluation.title")}>
          {tabs.map(([value, Icon, label]) => (
            <button
              ...
              className={`flex items-center gap-1.5 rounded-sm px-2.5 py-1.5 text-label ${
      ...
      {error && (
        <div role="alert" className="border-b border-destructive/40 px-4 py-2 text-label text-destructive">
          {error}
        </div>
      )}
        ...
        <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:grid-cols-[minmax(18rem,0.9fr)_minmax(20rem,1.1fr)] lg:overflow-hidden">
          <section className="overflow-y-auto border-r border-border p-4">
            ...
            {showProfileForm && (
              <div className="mt-3 grid gap-2 border-y border-border py-3">
                ...
                <button type="button" disabled={!parseObject(parametersJson) || !profileDraft.name.trim()} onClick={() => { const parameters = parseObject(parametersJson); if (parameters) void saveProfile({ ...profileDraft, parameters }).then((saved) => { if (saved) { setProfileId(saved.id); setShowProfileForm(false); } }); }} className="w-fit rounded-sm bg-primary px-3 py-1.5 text-label font-medium text-primary-foreground disabled:opacity-50">
                  {t("common.save")}
                </button>
              </div>
            )}
            ...
            <div className="mt-4 flex flex-col gap-2">
              <h3 className="text-label font-semibold text-foreground">{t("evaluation.variables")}</h3>
              {activeVersion?.variables.map((variable) => (
                <label key={variable.name} className="flex flex-col gap-1 text-label text-muted-foreground">
              ...
              {activeVersion?.variables.length === 0 && <span className="text-label text-muted-foreground">{t("evaluation.noVariables")}</span>}
            </div>
            <div className="mt-4 flex gap-2">
              <button type="button" disabled={!revisionId} onClick={() => void render(revisionId, inputs)} className="rounded-sm border border-input px-3 py-1.5 text-label text-foreground hover:bg-accent disabled:opacity-50">
                {t("evaluation.preview")}
              </button>
              {playgroundRequestId ? (
                <button type="button" onClick={() => void cancel("playground")} className="flex items-center gap-1.5 rounded-sm border border-input px-3 py-1.5 text-label text-foreground hover:bg-accent">
                  <SquareIcon className="h-3.5 w-3.5" aria-hidden="true" />{t("common.cancel")}
                </button>
              ) : (
                <button type="button" disabled={!revisionId || !profileId} onClick={() => void run({ promptRevisionId: revisionId, profileRevisionId: profileId, inputs })} className="flex items-center gap-1.5 rounded-sm bg-primary px-3 py-1.5 text-label font-medium text-primary-foreground disabled:opacity-50">
          ...
          <section className="grid min-h-0 grid-rows-2">
            <div className="overflow-y-auto border-b border-border p-4">
              <h3 className="mb-2 text-label font-semibold text-foreground">{t("evaluation.renderedInput")}</h3>
              {rendered?.messages.map((message, index) => (
                <div key={index} className="mb-2 grid grid-cols-[5rem_minmax(0,1fr)] gap-2 text-label">
            ...
            <div className="overflow-y-auto p-4" aria-live="polite">
              <h3 className="mb-2 text-label font-semibold text-foreground">{t("evaluation.output")}</h3>
              <pre className="whitespace-pre-wrap text-body text-foreground">{streamedOutput || t("evaluation.outputEmpty")}</pre>
            </div>
          </section>
        </div>
      )}
        ...
        <div className="flex min-h-0 flex-1 flex-col overflow-hidden">
          <div className="grid max-h-80 shrink-0 grid-cols-1 overflow-y-auto border-b border-border lg:max-h-none lg:grid-cols-3 lg:overflow-visible">
            <section className="border-r border-border p-3">
              ...
              <div className="mt-2 flex gap-1">
                <button type="button" disabled={!testSetName.trim() || testCases.length === 0} onClick={() => void saveTestSet({ name: testSetName, cases: testCases }).then((saved) => saved && setTestSetId(saved.id))} className="rounded-sm bg-primary px-2 py-1 text-label text-primary-foreground disabled:opacity-50">{t("common.save")}</button>
                <Select
                  value={testSetId}
              ...
              <div className="mt-1 flex gap-1">
                <button type="button" disabled={!testSetJson.trim()} onClick={() => void importTestSet(testSetJson).then((saved) => saved && setTestSetId(saved.id))} className="text-label text-muted-foreground hover:text-foreground disabled:opacity-50">{t("evaluation.import")}</button>
                <button type="button" disabled={!testSetId} onClick={() => void exportTestSet(testSetId).then((value) => value && setTestSetJson(value))} className="text-label text-muted-foreground hover:text-foreground disabled:opacity-50">{t("evaluation.export")}</button>
              </div>
            </section>

            <section className="border-r border-border p-3">
              <h3 className="text-label font-semibold">{t("evaluation.evaluators")}</h3>
              <div className="mt-2 flex max-h-20 flex-wrap gap-1 overflow-y-auto">
                {evaluators.map((evaluator) => (
                  <label key={evaluator.id} className="flex items-center gap-1 text-label text-muted-foreground">
              ...
              <button type="button" disabled={!evaluatorDraft.name.trim() || !parseObject(evaluatorConfigJson)} onClick={() => { const config = parseObject(evaluatorConfigJson); if (config) void createEvaluator({ ...evaluatorDraft, config }).then((saved) => saved && setSelectedEvaluators((ids) => [...ids, saved.id])); }} className="mt-1 rounded-sm border border-input px-2 py-1 text-label text-foreground hover:bg-accent disabled:opacity-50">{t("evaluation.addEvaluator")}</button>
            </section>

            <section className="p-3">
              <h3 className="text-label font-semibold">{t("evaluation.matrixScope")}</h3>
              <div className="mt-2 grid grid-cols-1 gap-2 xl:grid-cols-2">
                <div className="max-h-24 overflow-y-auto">
                  {versions.map((version) => <label key={version.id} className="flex items-center gap-1 text-label text-muted-foreground"><input type="checkbox" checked={selectedRevisions.includes(version.id)} onChange={() => toggle(selectedRevisions, version.id, setSelectedRevisions)} />v{version.version}</label>)}
                </div>
                <div className="max-h-24 overflow-y-auto">
                  {profiles.map((profile) => <label key={profile.id} className="flex items-center gap-1 text-label text-muted-foreground"><input type="checkbox" checked={selectedProfiles.includes(profile.id)} onChange={() => toggle(selectedProfiles, profile.id, setSelectedProfiles)} />{profile.name}</label>)}
                </div>
              </div>
              <div className="mt-3 flex items-center gap-2">
                {matrixRequestId ? <button type="button" onClick={() => void cancel("matrix")} className="flex items-center gap-1 rounded-sm border border-input px-2 py-1 text-label"><SquareIcon className="h-3 w-3" />{t("common.cancel")}</button> : <button type="button" disabled={!testSetId || selectedRevisions.length === 0 || selectedProfiles.length === 0 || selectedEvaluators.length === 0} onClick={() => void runMatrix({ testSetId, promptRevisionIds: selectedRevisions, profileRevisionIds: selectedProfiles, evaluatorIds: selectedEvaluators })} className="flex items-center gap-1 rounded-sm bg-primary px-2 py-1 text-label text-primary-foreground disabled:opacity-50"><PlayIcon className="h-3 w-3" />{t("evaluation.runMatrix")}</button>}
                {progress && <span aria-live="polite" className="text-label tabular-nums text-muted-foreground">{progress.completed}/{progress.total}</span>}
              </div>
            </section>
          </div>
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:grid-cols-[minmax(0,1fr)_22rem] lg:overflow-hidden">
            <div className="overflow-auto">
              <table className="w-full table-fixed border-collapse text-label">
              ...
              {!selectedMatrix && <div className="p-6 text-center text-label text-muted-foreground">{t("evaluation.matrixEmpty")}</div>}
            </div>
...
function ComparePanel({ cells, runById, prompt, labels, labelHistory, onManual, onLabel }: ComparePanelProps) {
  ...
  return (
    <aside className="overflow-y-auto border-t border-border p-3 lg:border-l lg:border-t-0">
      <h3 className="text-label font-semibold">{t("evaluation.compare")}</h3>
      {cells.length === 0 && <p className="mt-2 text-label text-muted-foreground">{t("evaluation.compareEmpty")}</p>}
      <div className={`mt-2 grid gap-2 ${cells.length === 2 ? "grid-cols-2" : "grid-cols-1"}`}>
        {cells.map((cell) => (
          <section key={cell.id} className="min-w-0 border-t border-border pt-2">
            <div className="flex items-center justify-between text-label"><span className="font-medium">{cell.status}</span>{cell.cacheHit && <span className="text-muted-foreground">{t("evaluation.cached")}</span>}</div>
            <pre className="mt-2 max-h-40 overflow-auto whitespace-pre-wrap text-label text-foreground">{cell.promptRunId ? runById.get(cell.promptRunId)?.output ?? cell.error ?? "-" : cell.error ?? "-"}</pre>
            ...
            {cell.status === "success" && (
              <div className="mt-2 flex flex-wrap gap-1">
                {(["baseline", "candidate"] as const).map((label) => <button key={label} type="button" onClick={() => onLabel(label, cell.promptRevisionId, false)} className="flex items-center gap-1 rounded-sm border border-input px-1.5 py-0.5 text-label"><TagIcon className="h-3 w-3" />{label}</button>)}
              </div>
            )}
          </section>
        ))}
      ...
      <div className="mt-4 border-t border-border pt-3">
        <h4 className="text-label font-semibold">{t("evaluation.labels")}</h4>
        {labels.map((label) => <div key={label.label} className="mt-1 flex items-center justify-between text-label"><span>{label.label}: {label.promptRevisionId.slice(0, 8)}</span></div>)}
        {labelHistory.slice(0, 5).map((item) => <button key={item.id} type="button" disabled={!item.fromRevisionId} onClick={() => item.fromRevisionId && onLabel(item.label, item.fromRevisionId, true)} className="mt-1 block text-left text-label text-muted-foreground hover:text-foreground disabled:opacity-50">{new Date(item.createdAt).toLocaleString()} · {item.action ?? "move"} · {item.label}</button>)}
        {labels.length === 0 && <span className="text-label text-muted-foreground">{prompt.title}: {t("evaluation.noLabels")}</span>}
      </div>
    </aside>
  );
}
