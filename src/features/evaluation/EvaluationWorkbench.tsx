...
const inputClass =
  "w-full rounded border border-input bg-background px-2 py-1.5 text-xs text-foreground outline-none focus:ring-1 focus:ring-ring";
          ...
          <section className="overflow-y-auto border-r border-border p-4">
            ...
            {showProfileForm && (
              <div className="mt-3 grid gap-2 border-y border-border py-3">
                ...
                <button type="button" disabled={!parseObject(parametersJson) || !profileDraft.name.trim()} onClick={() => { const parameters = parseObject(parametersJson); if (parameters) void saveProfile({ ...profileDraft, parameters }).then((saved) => { if (saved) { setProfileId(saved.id); setShowProfileForm(false); } }); }} className="w-fit rounded bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground disabled:opacity-40">
                  {t("common.save")}
                </button>
              </div>
            )}
            ...
            <div className="mt-4 flex gap-2">
              ...
              {activeRequestId ? (
                ...
                <button type="button" disabled={!revisionId || !profileId} onClick={() => void run({ promptRevisionId: revisionId, profileRevisionId: profileId, inputs })} className="flex items-center gap-1.5 rounded bg-primary px-3 py-1.5 text-xs font-medium text-primary-foreground disabled:opacity-40">
          ...
          <div className="grid max-h-80 shrink-0 grid-cols-1 overflow-y-auto border-b border-border lg:max-h-none lg:grid-cols-3 lg:overflow-visible">
            <section className="border-r border-border p-3">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={addCase} aria-label={t("evaluation.addCase")} title={t("evaluation.addCase")} className="rounded p-1 text-muted-foreground hover:bg-accent"><PlusIcon className="h-3.5 w-3.5" /></button>
              </div>
              ...
              <div className="mt-2 max-h-32 space-y-2 overflow-y-auto">
                {testCases.map((testCase, index) => (
                  <div key={index} className="grid grid-cols-[1fr_1.4fr_auto] gap-1">
                    ...
                    <button type="button" onClick={() => setTestCases((cases) => cases.filter((_, itemIndex) => itemIndex !== index))} aria-label={t("evaluation.removeCase")} title={t("evaluation.removeCase")} className="rounded p-1 text-muted-foreground hover:bg-accent"><XIcon className="h-3.5 w-3.5" /></button>
                  </div>
                ))}
              </div>
              <div className="mt-2 flex gap-1">
                <button type="button" disabled={!testSetName.trim() || testCases.length === 0} onClick={() => void saveTestSet({ name: testSetName, cases: testCases }).then((saved) => saved && setTestSetId(saved.id))} className="rounded bg-primary px-2 py-1 text-xs text-primary-foreground disabled:opacity-40">{t("common.save")}</button>
              ...
              <div className="mt-1 flex gap-1">
                <button type="button" disabled={!testSetJson.trim()} onClick={() => void importTestSet(testSetJson).then((saved) => saved && setTestSetId(saved.id))} className="text-xs text-muted-foreground hover:text-foreground disabled:opacity-40">{t("evaluation.import")}</button>
                <button type="button" disabled={!testSetId} onClick={() => void exportTestSet(testSetId).then((value) => value && setTestSetJson(value))} className="text-xs text-muted-foreground hover:text-foreground disabled:opacity-40">{t("evaluation.export")}</button>
              </div>
            </section>
                  ...
                  {(selectedMatrix?.cells ?? []).map((cell) => (
                    <tr key={cell.id}>
                      ...
                      <td className="border-b border-border p-1">
                        <button type="button" aria-pressed={compareCells.includes(cell.id)} onClick={() => setCompareCells((items) => items.includes(cell.id) ? items.filter((id) => id !== cell.id) : [...items.slice(-1), cell.id])} className={`flex w-full items-center justify-between rounded border px-2 py-1.5 text-left ${compareCells.includes(cell.id) ? "border-primary bg-primary/10" : "border-input hover:bg-accent"}`}>
        ...
        <div className="flex min-h-0 flex-1 flex-col">
          ...
          <div className="grid min-h-0 flex-1 grid-cols-1 overflow-y-auto lg:grid-cols-[minmax(0,1fr)_18rem] lg:overflow-hidden">
            ...
            <aside className="overflow-y-auto border-t border-border p-3 lg:border-l lg:border-t-0">
              <div className="flex items-center justify-between"><h3 className="text-xs font-semibold">{t("evaluation.matrixRuns")}</h3>{selectedMatrix && <button type="button" onClick={() => void retryMatrix(selectedMatrix.run.id)} aria-label={t("evaluation.retry")} title={t("evaluation.retry")} className="rounded p-1 text-muted-foreground hover:bg-accent"><RefreshCwIcon className="h-3.5 w-3.5" /></button>}</div>
              {matrices.map((matrix) => <button key={matrix.id} type="button" onClick={() => void selectMatrix(matrix.id)} className="mt-2 block w-full rounded border border-input p-2 text-left text-xs hover:bg-accent"><span className="block text-foreground">{matrix.status} · {matrix.completedCells}/{matrix.totalCells}</span><span className="text-muted-foreground">{new Date(matrix.startedAt).toLocaleString()}</span></button>)}
            </aside>
          </div>
        </div>
      )}
            ...
            <div className="mt-2 space-y-2">
              {cell.results.map((result) => (
                <div key={result.evaluatorId} className="border-t border-border pt-2 text-xs">
                  ...
                  {result.kind === "manual" && result.skipped && <div className="mt-1 flex gap-1"><button type="button" onClick={() => onManual(cell.id, result.evaluatorId, true)} className="rounded border border-input px-1.5 py-0.5">{t("evaluation.pass")}</button><button type="button" onClick={() => onManual(cell.id, result.evaluatorId, false)} className="rounded border border-input px-1.5 py-0.5">{t("evaluation.fail")}</button></div>}
                </div>
              ))}
            </div>
            {cell.status === "success" && (
              <div className="mt-2 flex flex-wrap gap-1">
                {(["baseline", "candidate"] as const).map((label) => <button key={label} type="button" onClick={() => onLabel(label, cell.promptRevisionId, false)} className="flex items-center gap-1 rounded border border-input px-1.5 py-0.5 text-xs"><TagIcon className="h-3 w-3" />{label}</button>)}
              </div>
            )}
          </section>
        ))}
